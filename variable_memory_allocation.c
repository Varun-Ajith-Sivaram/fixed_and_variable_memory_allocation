...
void main()
{
    printf("\n\tImplementation of Memory Allocation Methods for Variable Partition\n\n");
...
void get_memory_details(struct mblock memory[])
{
    printf("\n   Enter the details of the memory blocks: \n");
        ...
        for(i = 0; i < total_memory; i++)
        {
            ...
            memory[i].rem_size = memory[i].msize;
        }
...
void allocate_first_fit(struct pblock process[],struct mblock memory[])
{
    for(i = 0; i < total_process; i++)
    {
        ...
        {
            if(process[i].psize <= memory[j].rem_size)
            {
                process[i].alloc_mblock = memory[j].block_no;
                memory[j].rem_size -= process[i].psize;
                break;
            }
        }
    }
...
void allocate_best_fit(struct pblock process[],struct mblock memory[])
{
    ...
    for(i = 0; i < total_process; i++)
    {
        for(j = 0; j < total_memory; j++)
        {
            if(process[i].psize <= memory[j].rem_size)
            {
               min_size = memory[j].rem_size;
               min_block_no = memory[j].block_no;
               min_index = j;
                   
               for(k = j+1; k < total_memory; k++)
               {
                   if(process[i].psize <= memory[k].rem_size && min_size > memory[k].rem_size)
                   {
                       min_size = memory[k].rem_size;
                ...
                memory[min_index].rem_size -= process[i].psize;
               break;
            }
        }
    }
...
void allocate_worst_fit(struct pblock process[],struct mblock memory[])
{
    ...
    for(i = 0; i < total_process; i++)
    {
        for(j = 0; j < total_memory; j++)
        {
            if(process[i].psize <= memory[j].rem_size)
            {
                max_size = memory[j].rem_size;
                max_block_no = memory[j].block_no;
                max_index = j;
                   
                for(k = j+1; k < total_memory; k++)
                {
                   if(process[i].psize <= memory[k].rem_size && max_size < memory[k].rem_size)
                   {
                       max_size = memory[k].rem_size;
                ...
                memory[max_index].rem_size -= process[i].psize;
                break;
